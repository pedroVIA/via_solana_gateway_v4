...
describe("End-to-End Tests - Security Scenarios", () => {
  ...
  afterEach(async () => {
    await Promise.all([context.teardown(), attackerContext.teardown()]);
  });
  ...
  it("[E2E-018] should prevent unauthorized gateway operations", async () => {
    ...
    const unauthorizedAuthority = await createFundedKeypair(
      context.connection,
      TEST_CONFIG.AIRDROP_AMOUNT
    );
    const unauthorizedRelayer = await createFundedKeypair(
      context.connection,
      TEST_CONFIG.AIRDROP_AMOUNT
    );
  ...
  it("[E2E-019] should handle replay attack scenarios robustly", async () => {
    ...
    const pdaClosed = !(await context.txIdPDAExists(
      advancedSourceChain,
      advancedTxId
    ));
  ...
  it("[E2E-020] should resist resource exhaustion attacks", async () => {
    ...
    for (let batch = 0; batch < exhaustionAttack.concurrent_batches; batch++) {
      ...
      for (
        let attempt = 0;
        attempt < exhaustionAttack.attempts_per_batch;
        attempt++
      ) {
        ...
        batchPromises.push(
          context
            .createTxPda(attackTxId, exhaustionAttack.attack_source_chain)
            .then(() => {
    ...
    const legitOpsSuccess = legitimateOperations.filter(
      (op) => op.success
    ).length;

    logSuccess(
      `Legitimate operations during attack: ${legitOpsSuccess}/10 successful`
    );
    console.log(`  Duration: ${legitOpsDuration}ms`);
    console.log(
      `  Success Rate: ${((legitOpsSuccess / 10) * 100).toFixed(1)}%`
    );
    ...
    const successfulAttackTxIds = batchResults
      .flat()
      .filter((id) => id !== null);

    for (const attackTxId of successfulAttackTxIds.slice(0, 20)) {
      // Clean up first 20
      try {
        await context.processMessage(
          attackTxId,
    ...
    logSuccess(
      `Attack cleanup: ${cleanupResults.processed} PDAs processed, ${cleanupResults.errors} errors`
    );
  ...
  it("[E2E-021] should prevent cross-chain message tampering", async () => {
    ...
    for (const test of tamperingTests) {
      ...
      if (test.expectedError) {
        ...
        logSuccess(
          `${test.name}: Correctly rejected with ${test.expectedError}`
        );
      } else {
        ...
        logSuccess(
          `${test.name}: Processed (payload integrity depends on application logic)`
        );
      }
    }
    ...
    logSuccess(
      "Cross-chain consistency: Transaction succeeds but payload validation is application responsibility"
    );
  ...
  it("[E2E-022] should handle sophisticated timing attacks", async () => {
    ...
    for (let i = 0; i < timingAttack.concurrent_attempts; i++) {
      racePromises.push(
        context
          .createTxPda(
            timingAttack.target_tx_id,
            timingAttack.target_source_chain
          )
          .then((tx) => {
    ...
    expect(raceResults.failed_creates).to.equal(
      timingAttack.concurrent_attempts - 1
    );
    ...
    processingTests.forEach((test, i) => {
      console.log(
        `  Test ${i}: TX1=${test.tx1_time}ms, TX2=${test.tx2_time}ms, Total=${test.total_time}ms`
      );
    });
    ...
    console.log(
      `  Exploitation Attempts: ${stateTimingTest.exploitation_attempts}`
    );
    console.log(
      `  Successful Exploits: ${stateTimingTest.successful_exploits}`
    );
  ...
  it("[E2E-023] should maintain security under system stress conditions", async () => {
    ...
    console.log(
      `  Security Check Interval: ${stressTest.security_checks_interval_ms}ms`
    );
    ...
    const securityMonitoring = async () => {
      while (
        Date.now() - stressResults.start_time <
        stressTest.stress_duration_ms
      ) {
        try {
          // Security health check
          const healthTxId = new BN(Date.now() + Math.random() * 1000000);
          const healthSourceChain = new BN(
            9000 + Math.floor(Math.random() * 10)
          );
          ...
          console.log(
            `Security check ${stressResults.system_health_checks} failed: ${error}`
          );
        }
    ...
    await Promise.all([securityMonitoring(), stressLoad()]);
    ...
    console.log(
      `  Success Rate: ${(
        (stressResults.successful_operations / stressResults.total_operations) *
        100
      ).toFixed(1)}%`
    );
    ...
    const postStressSuccess = postStressChecks.filter(
      (check) => check.success
    ).length;

    logSuccess(
      `Post-stress verification: ${postStressSuccess}/5 checks successful`
    );

    // Security assertions
    expect(stressResults.security_violations).to.be.lessThan(
      stressResults.system_health_checks * 0.1
    ); // Less than 10% violations
    expect(stressResults.successful_operations).to.be.greaterThan(
      stressResults.total_operations * 0.7
    ); // At least 70% success
