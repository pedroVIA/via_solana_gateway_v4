...
describe("End-to-End Tests - Cross-Chain Message Flows", () => {
  ...
  it("[E2E-001] should simulate complete Ethereum → Solana message flow", async () => {
    ...
    const initialMetrics = {
      ...
      relayerBalance: await context.connection.getBalance(
        context.relayer!.publicKey
      ),
    };

    logSuccess(
      `Simulating bridge of ${ethereumTxData.amount} ${ethereumTxData.token}`
    );
    logSuccess(
      `From Ethereum chain ${sourceChainId} to Solana chain ${destChainId}`
    );
    ...
    if (counterAfterTx1) {
      expect(counterAfterTx1.sourceChainId.toString()).to.equal(
        sourceChainId.toString()
      );
      logSuccess(
        `Counter updated - Highest TX ID: ${counterAfterTx1.highestTxIdSeen.toString()}`
      );
    }
    ...
    const txDelta =
      finalMetrics.transactionCount - initialMetrics.transactionCount;
    const cuDelta = finalMetrics.computeUnits - initialMetrics.computeUnits;
    const balanceDelta =
      (finalMetrics.relayerBalance - initialMetrics.relayerBalance) /
    ...
    console.log(
      `  Message: ${ethereumTxData.action} ${ethereumTxData.amount} ${ethereumTxData.token}`
    );
  });
  ...
  it("[E2E-002] should handle bidirectional cross-chain messaging", async () => {
    ...
    logSuccess(
      `ETH → SOL: Transferred ${eth2solData.amount} ${eth2solData.token}`
    );
    ...
    logSuccess(
      `SOL → ETH: Confirmed ${sol2ethData.amount} ${sol2ethData.token}`
    );

    // Verify both transactions completed
    const eth2solExists = await context.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      eth2solTxId
    );
    const sol2ethExists = await context.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      sol2ethTxId
    );
    ...
    if (ethCounter) {
      expect(ethCounter.sourceChainId.toString()).to.equal(
        CHAIN_IDS.ETHEREUM_MAINNET.toString()
      );
      logSuccess(
        `ETH counter - Highest TX: ${ethCounter.highestTxIdSeen.toString()}`
      );
    }
  ...
  it("[E2E-003] should handle complex multi-chain routing scenarios", async () => {
    ...
    logSuccess(
      `Stage 1 complete: ${routingData.amount} ${routingData.asset} from ETH to SOL`
    );
    ...
    logSuccess(
      `Stage 2 complete: ${routingData.amount} ${routingData.asset} from SOL to BSC`
    );

    // Verify routing completed
    const stage1Exists = await context.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      stage1TxId
    );
    const stage2Exists = await context.txIdPDAExists(
    ...
    if (ethCounter && bscCounter) {
      ...
      expect(bscCounter.sourceChainId.toString()).to.equal(
        CHAIN_IDS.BSC_MAINNET.toString()
      );
    ...
    console.log(
      `  Stages: ${stage1TxId.toString()} → ${stage2TxId.toString()}`
    );
  });
  ...
  it("[E2E-004] should simulate high-frequency cross-chain arbitrage", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const operation = {
        id: baseTime + i * 100,
        sourceChain:
          i % 2 === 0 ? CHAIN_IDS.ETHEREUM_MAINNET : CHAIN_IDS.POLYGON_MAINNET,
    ...
    console.log(
      `  Throughput: ${(arbitrageOps.length / (totalDuration / 1000)).toFixed(
        2
      )} ops/sec`
    );
  ...
  it("[E2E-005] should handle cross-chain governance proposals", async () => {
    ...
    if (govCounter) {
      expect(govCounter.sourceChainId.toString()).to.equal(
        sourceChain.toString()
      );
      logSuccess(
        `Governance counter updated: ${govCounter.highestTxIdSeen.toString()}`
      );
    }
    ...
    console.log(
      `  Affected Chains: ${governanceProposal.parameters.chains_affected.join(
        ", "
      )}`
    );
  });
  ...
  it("[E2E-006] should simulate real-world DeFi protocol integration", async () => {
    ...
    logSuccess(
      `DeFi Operation: ${defiOperation.source_protocol} → ${defiOperation.dest_protocol}`
    );
    ...
    logSuccess(
      `Deposited to Raydium pool with ${depositData.expected_apy} APY`
    );

    // Verify both operations completed
    const withdrawExists = await context.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      withdrawTxId
    );
    const depositExists = await context.txIdPDAExists(
      context.chainId,
      depositTxId
    );
    ...
    console.log(
      `  Route: ${defiOperation.source_protocol} → ${defiOperation.dest_protocol}`
    );
    console.log(
      `  Total Value: ${(totalValue / 1000000).toLocaleString()} USD`
    );
    console.log(
      `  Assets: ${defiOperation.assets
