...
describe("End-to-End Tests - Multi-Gateway Interactions", () => {
  ...
  it("[E2E-007] should handle concurrent messages across multiple gateways", async () => {
    ...
    console.log(
      `  Primary Gateway (Chain ${primaryGateway.chainId}): ${primaryGateway.gatewayPDA}`
    );
    console.log(
      `  Secondary Gateway (Chain ${secondaryGateway.chainId}): ${secondaryGateway.gatewayPDA}`
    );
    console.log(
      `  Tertiary Gateway (Chain ${tertiaryGateway.chainId}): ${tertiaryGateway.gatewayPDA}`
    );
    ...
    logSuccess(
      `TX1 phase completed in ${tx1Duration}ms across ${messages.length} gateways`
    );
    ...
    for (const msg of messages) {
      const exists = await msg.gateway.txIdPDAExists(
        msg.sourceChain,
        new BN(msg.id)
      );
    ...
    logSuccess(
      `TX2 phase completed in ${tx2Duration}ms across ${messages.length} gateways`
    );
    ...
    console.log(
      `  Average per Message: ${(totalDuration / messages.length).toFixed(0)}ms`
    );
  });
  ...
  it("[E2E-008] should handle gateway-to-gateway message routing", async () => {
    ...
    const routingScenario = {
      ...
      route: [
        {
          gateway: primaryGateway,
          chain: CHAIN_IDS.ETHEREUM_MAINNET,
          role: "source",
        },
        {
          gateway: secondaryGateway,
          chain: secondaryGateway.chainId,
          role: "intermediate",
        },
        {
          gateway: tertiaryGateway,
          chain: tertiaryGateway.chainId,
          role: "destination",
        },
      ],
      payload: {
    ...
    logSuccess(
      `Routing ${routingScenario.payload.asset} through ${routingScenario.route.length} gateways`
    );
    ...
    logSuccess(
      `Hop 2 complete: Final destination gateway ${tertiaryGateway.chainId}`
    );
    ...
    const hop1Exists = await primaryGateway.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      hop1TxId
    );
    const hop2Exists = await secondaryGateway.txIdPDAExists(
      secondaryGateway.chainId,
      hop2TxId
    );
    const finalExists = await tertiaryGateway.txIdPDAExists(
      tertiaryGateway.chainId,
      finalTxId
    );
    ...
    const primaryCounter = await primaryGateway.getCounterPDA(
      CHAIN_IDS.ETHEREUM_MAINNET
    );
    const secondaryCounter = await secondaryGateway.getCounterPDA(
      secondaryGateway.chainId
    );
    const tertiaryCounter = await tertiaryGateway.getCounterPDA(
      tertiaryGateway.chainId
    );

    logSuccess("Inter-gateway routing completed successfully");
    console.log(
      `  Route: ${routingScenario.route.map((r) => r.role).join(" → ")}`
    );
    console.log(
      `  Asset: ${routingScenario.payload.amount} ${routingScenario.payload.asset}`
    );
    console.log(`  Hops: ${hop1TxId} → ${hop2TxId} → ${finalTxId}`);

    if (primaryCounter)
      console.log(`  Primary Counter: ${primaryCounter.highestTxIdSeen}`);
    if (secondaryCounter)
      console.log(`  Secondary Counter: ${secondaryCounter.highestTxIdSeen}`);
    if (tertiaryCounter)
      console.log(`  Tertiary Counter: ${tertiaryCounter.highestTxIdSeen}`);
  });
  ...
  it("[E2E-009] should handle gateway synchronization across chain states", async () => {
    ...
    const syncScenario = {
      ...
      involved_gateways: [
        ...
        {
          gateway: secondaryGateway,
          chain: secondaryGateway.chainId.toString(),
        },
    ...
    healthChecks.forEach((gateway, i) => {
      ...
      expect(gateway.chainId.toString()).to.equal(
        syncScenario.involved_gateways[i].chain
      );
    });
  ...
  it("[E2E-010] should handle gateway failover scenarios", async () => {
    ...
    await secondaryGateway.createTxPda(
      failoverTxId,
      CHAIN_IDS.ETHEREUM_MAINNET
    );
    ...
    const normalExists = await primaryGateway.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      normalTxId
    );
    const failedExists = await primaryGateway.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      failedTxId
    );
    const failoverExists = await secondaryGateway.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      failoverTxId
    );
    const recoveryExists = await primaryGateway.txIdPDAExists(
      CHAIN_IDS.ETHEREUM_MAINNET,
      recoveryTxId
    );
  ...
  it("[E2E-011] should handle complex multi-gateway consensus mechanisms", async () => {
    ...
    logSuccess(
      `Required Approvals: ${consensusOperation.minimum_approvals}/${consensusOperation.participating_gateways.length}`
    );

    // Phase 1: Submit consensus proposal to all gateways
    logSubtest("Phase 1: Submit consensus proposals");
    const proposalPromises = consensusOperation.participating_gateways.map(
      (gw, index) => {
        const proposalTxId = new BN(
          consensusOperation.operation_id + (index + 1) * 100
        );
    ...
    const approvals = consensusOperation.participating_gateways.filter(
      (gw) => gw.vote === "approve"
    ).length;
    ...
    logSuccess(
      `Consensus reached: ${approvals}/${consensusOperation.participating_gateways.length} approvals`
    );
    ...
    for (const proposal of proposalPromises) {
      const exists = await proposal.gateway.txIdPDAExists(
    ...
    const executionExists = await primaryGateway.txIdPDAExists(
      primaryGateway.chainId,
      executionTxId
    );
    ...
    console.log(
      `  Consensus Result: ${approvals}/${consensusOperation.participating_gateways.length} ✅`
    );
    console.log(`  Execution TX: ${executionTx}`);
    console.log(
      `  New Bridge Fee: ${consensusOperation.proposed_changes.bridge_fee_bps} BPS`
    );
    console.log(
      `  New Max TX Size: ${consensusOperation.proposed_changes.max_transaction_size}`
    );
  });
  ...
  it("[E2E-012] should handle massive parallel processing across gateways", async () => {
    ...
    logSuccess(
      `Generating ${massProcessingTest.total_messages} messages across ${massProcessingTest.gateways.length} gateways`
    );
    ...
    for (let i = 0; i < massProcessingTest.total_messages; i++) {
      ...
      allMessages.push({
        ...
        data: {
          ...
          payload_type:
            i % 3 === 0 ? "transfer" : i % 3 === 1 ? "swap" : "bridge",
    ...
    logSuccess(
      `TX1 batch completed: ${createResults.length} PDAs in ${createDuration}ms`
    );
    console.log(
      `  Average TX1 time: ${(createDuration / createResults.length).toFixed(
        1
      )}ms`
    );
    ...
    logSuccess(
      `TX2 batch completed: ${processResults.length} messages in ${processDuration}ms`
    );
    console.log(
      `  Average TX2 time: ${(processDuration / processResults.length).toFixed(
        1
      )}ms`
    );
    ...
    const gatewayMetrics = massProcessingTest.gateways.map((gateway, index) => {
      const gatewayMessages = allMessages.filter(
        (msg) => msg.gatewayIndex === index
      );
      return {
        gateway_id: gateway.chainId.toString(),
    ...
    gatewayMetrics.forEach((metrics, i) => {
      console.log(
        `  Gateway ${i + 1} (${metrics.gateway_id}): ${
          metrics.messages_processed
        } msgs, ${metrics.total_transactions} total TXs`
      );
    });
