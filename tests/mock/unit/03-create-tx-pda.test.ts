...
describe("Unit Tests - Create TX PDA", () => {
  ...
  it("[CREATE-001] should create TxId PDA with valid parameters and counter tracking", async () => {
    ...
    expect(counterPDA.sourceChainId.toString()).to.equal(
  ...
  it("[CREATE-003] should handle different source chain IDs with isolation", async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      expect(counterPDA.sourceChainId.toString()).to.equal(
        testCase.chainId.toString()
      );
      expect(counterPDA.highestTxIdSeen.toString()).to.equal(
        uniqueTxId.toString()
      );

      console.log(`  ✓ ${testCase.name}: TX ${uniqueTxId.toString()} -> ${tx}`);
    }
  ...
  it("[CREATE-004] should update counter PDA with highest tx_id seen", async () => {
    ...
    for (let i = 0; i < txIds.length; i++) {
      ...
      const counterPDA = await context.getCounterPDA(sourceChainId);
      expect(counterPDA.highestTxIdSeen.toString()).to.equal(
        expectedHighest.toString()
      );

      console.log(
        `  ✓ TX ${txId.toString()} -> Highest: ${counterPDA.highestTxIdSeen.toString()}`
      );
    }
  ...
  it("[CREATE-005] should create separate counters for different source chains", async () => {
    ...
    expect(pdas1.counterPDA.toString()).to.not.equal(
      pdas2.counterPDA.toString()
    );

    logSuccess("Separate counter PDAs created for different chains");
    console.log(
      `  Chain ${chain1.toString()} Counter: ${pdas1.counterPDA.toString()}`
    );
    console.log(
      `  Chain ${chain2.toString()} Counter: ${pdas2.counterPDA.toString()}`
    );
  });
  ...
  it("[CREATE-006] should allow same tx_id for different source chains", async () => {
    ...
    console.log(`  TX ID: ${txId.toString()}`);
    console.log(
      `  Chain ${chain1.toString()} PDA: ${pdas1.txIdPDA.toString()}`
    );
    console.log(
      `  Chain ${chain2.toString()} PDA: ${pdas2.txIdPDA.toString()}`
    );
  });
  ...
  it("[CREATE-007] should handle large transaction IDs", async () => {
    ...
    expect(cuUsed).to.be.lessThan(
      CU_LIMITS.EXPECTED_CREATE_TX_PDA * 1.5,
      `Large TX ID used ${cuUsed} CU, expected <${
        CU_LIMITS.EXPECTED_CREATE_TX_PDA * 1.5
      }`
    );
  ...
  it("[CREATE-008] should validate PDA derivation is correct", async () => {
    ...
    const accountInfo1 = await context.connection.getAccountInfo(
      expectedPDAs.txIdPDA
    );
    const accountInfo2 = await context.connection.getAccountInfo(
      expectedPDAs.counterPDA
    );
    ...
    expect(accountInfo1!.owner.toString()).to.equal(
      context.program.programId.toString()
    );
    expect(accountInfo2!.owner.toString()).to.equal(
      context.program.programId.toString()
    );
  ...
  it("[CREATE-009] should work with different relayer accounts", async () => {
    ...
    const tx = await context.createTxPda(
      txId,
      sourceChainId,
      context.unauthorizedUser!
    );
    ...
    expect(cuUsed).to.be.lessThan(
      CU_LIMITS.EXPECTED_CREATE_TX_PDA * 1.5,
      `Different relayer used ${cuUsed} CU, expected <${
        CU_LIMITS.EXPECTED_CREATE_TX_PDA * 1.5
      }`
    );
  ...
  it("[CREATE-010] should preserve existing counter when creating new TxId PDA", async () => {
    logTestHeader(
      "[CREATE-010] Counter Preservation Across Multiple TxId PDAs"
    );
    context.showContext();
