...
describe("Unit Tests - Process Message", () => {
  ...
  it("[PROCESS-001] should process a valid message and close TxId PDA atomically", async () => {
    logTestHeader(
      "[PROCESS-001] Valid Message Processing with Atomic PDA Closure"
    );
    context.showContext();
    ...
    const relayerBalanceBefore = await context.connection.getBalance(
    ...
    const relayerBalanceAfter = await context.connection.getBalance(
      context.relayer!.publicKey
    );
    ...
    console.log(
      `  Rent Reclaimed: ${relayerBalanceAfter - relayerBalanceBefore} lamports`
    );
  });
  ...
  it("[PROCESS-007] should handle moderately large on-chain data", async () => {
    ...
    expect(cuUsed).to.be.lessThan(
      CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5,
      `Large on-chain data used ${cuUsed} CU, expected <${
        CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5
      }`
    );
  ...
  it("[PROCESS-008] should handle moderately large off-chain data", async () => {
    ...
    expect(cuUsed).to.be.lessThan(
      CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5,
      `Large off-chain data used ${cuUsed} CU, expected <${
        CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5
      }`
    );
  ...
  it("[PROCESS-009] should handle large payloads correctly", async () => {
    ...
    expect(cuUsed).to.be.lessThan(
      CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5,
      `Large payloads used ${cuUsed} CU, expected <${
        CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5
      }`
    );
  ...
  it("[PROCESS-011] should work with different relayer accounts", async () => {
    ...
    const balanceBefore = await context.connection.getBalance(
      context.unauthorizedUser!.publicKey
    );
    ...
    const balanceAfter = await context.connection.getBalance(
      context.unauthorizedUser!.publicKey
    );
  ...
  it("[PROCESS-012] should validate tx_id matches TxId PDA", async () => {
    ...
    logSuccess(
      "TX ID validation correctly enforced - wrong tx_id targets non-existent PDA"
    );
  ...
  it("[PROCESS-013] should process messages from different source chains", async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      console.log(
        `  ✓ ${testCase.name}: TX ${txId.toString()} -> ${processTx}`
      );
    }
