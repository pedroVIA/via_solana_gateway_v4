...
describe("Integration Tests - Performance & Compute Units", () => {
  ...
  it("[PERF-001] should track compute units for complete TX1 → TX2 flow", async () => {
    ...
    console.log(
      `  Average per transaction: ${Math.round(totalDuration / totalTxCount)}ms`
    );
  ...
  it("[PERF-002] should handle high-throughput message processing", async () => {
    ...
    console.log(
      `  Throughput: ${((messageCount / totalDuration) * 1000).toFixed(
        2
      )} messages/sec`
    );
  });
  ...
  it("[PERF-003] should measure rent economics for PDA lifecycle", async () => {
    ...
    const balanceInitial = await context.connection.getBalance(
      context.relayer!.publicKey
    );

    // TX1: Create TxId PDA (costs rent)
    const tx1 = await context.createTxPda(txId, sourceChainId);
    const balanceAfterTx1 = await context.connection.getBalance(
      context.relayer!.publicKey
    );
    const tx1Cost =
      (balanceInitial - balanceAfterTx1) / anchor.web3.LAMPORTS_PER_SOL;
    ...
    const balanceFinal = await context.connection.getBalance(
      context.relayer!.publicKey
    );
    const tx2Benefit =
      (balanceFinal - balanceAfterTx1) / anchor.web3.LAMPORTS_PER_SOL;
    ...
    const netCost =
      (balanceInitial - balanceFinal) / anchor.web3.LAMPORTS_PER_SOL;
  ...
  it("[PERF-006] should validate system performance under load", async () => {
    ...
    const startTime = Date.now();
    const initialTxCount = context.metrics.transactionCount;
    const initialBalance = await context.connection.getBalance(
      context.relayer!.publicKey
    );
    ...
    const totalDuration = Date.now() - startTime;
    const totalTxCount = context.metrics.transactionCount - initialTxCount;
    const finalBalance = await context.connection.getBalance(
      context.relayer!.publicKey
    );
    const netCost =
    ...
    console.log(`  Total duration: ${totalDuration}ms`);
    console.log(
      `  Average per message: ${Math.round(totalDuration / loadTestCount)}ms`
    );
    console.log(
      `  Throughput: ${((loadTestCount / totalDuration) * 1000).toFixed(
        2
      )} messages/sec`
    );
    console.log(`  Net cost: ${netCost.toFixed(6)} SOL`);
    console.log(
      `  Cost per message: ${(netCost / loadTestCount).toFixed(8)} SOL`
    );
  });
});
