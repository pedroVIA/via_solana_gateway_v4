...
describe("Integration Tests - Error Recovery & Edge Cases", () => {
  ...
  it("[ERROR-001] should recover from interrupted TX1 → TX2 flow", async () => {
    ...
    const txIdExistsAfterInterruption = await context.txIdPDAExists(sourceChainId, txId);
    ...
    const txIdExistsAfterRecovery = await context.txIdPDAExists(sourceChainId, txId);
  ...
  it("[ERROR-002] should handle partial batch failures gracefully", async () => {
    ...
    const message1Exists = await context.txIdPDAExists(sourceChainId, messages[0].txId);
    const message2Exists = await context.txIdPDAExists(sourceChainId, messages[1].txId);
    const message3Exists = await context.txIdPDAExists(sourceChainId, messages[2].txId);
  ...
  it("[ERROR-003] should handle chain ID mismatches gracefully", async () => {
    ...
    console.log(`  Wrong Dest Chain: ${wrongDestChainId.toString()} - REJECTED ❌`);
    console.log(`  Correct Dest Chain: ${correctDestChainId.toString()} - ACCEPTED ✅`);
    console.log(`  Note: Dest chain validation is now enforced at program level`);
  });
  ...
  it("[ERROR-004] should handle multiple gateway interactions", async () => {
    ...
    try {
      ...
      console.log(`  Gateway 1 Status: ${gateway1State.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
      console.log(`  Gateway 2 Status: ${gateway2State.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
