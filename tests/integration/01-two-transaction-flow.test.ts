...
describe("Integration Tests - Two-Transaction Flow", () => {
  ...
  it("[FLOW-001] should complete full TX1 → TX2 flow successfully", async () => {
    ...
    const relayerBalanceInitial = await context.connection.getBalance(context.relayer!.publicKey);
    logSuccess(`Initial relayer balance: ${relayerBalanceInitial / anchor.web3.LAMPORTS_PER_SOL} SOL`);
    ...
    if (counterAfterTx1) {
      ...
      logSuccess(`Highest TX ID seen: ${counterAfterTx1.highestTxIdSeen.toString()}`);
    } else {
    ...
    const relayerBalanceFinal = await context.connection.getBalance(context.relayer!.publicKey);
    const balanceChange = (relayerBalanceFinal - relayerBalanceInitial) / anchor.web3.LAMPORTS_PER_SOL;
    ...
    expect(cuUsed1).to.be.lessThan(CU_LIMITS.EXPECTED_CREATE_TX_PDA * 1.5, 
      `TX1 used ${cuUsed1} CU, expected <${CU_LIMITS.EXPECTED_CREATE_TX_PDA * 1.5}`);
    expect(cuUsed2).to.be.lessThan(CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5, 
      `TX2 used ${cuUsed2} CU, expected <${CU_LIMITS.EXPECTED_PROCESS_MESSAGE * 1.5}`);
    ...
    if (counterAfterTx2 && counterAfterTx1) {
      expect(counterAfterTx2.highestTxIdSeen.toString()).to.equal(counterAfterTx1.highestTxIdSeen.toString());
    }
  ...
  it("[FLOW-004] should handle multiple concurrent TX1s for different messages", async () => {
    ...
    if (counter) {
      ...
      console.log(`  Highest TX ID seen: ${counter.highestTxIdSeen.toString()}`);
    }
  ...
  it("[FLOW-005] should complete multiple full flows for different messages", async () => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const txIdExists = await context.txIdPDAExists(sourceChainId, message.txId);
    ...
    if (finalCounter) {
      ...
      console.log(`  Final highest TX ID: ${finalCounter.highestTxIdSeen.toString()}`);
    }
