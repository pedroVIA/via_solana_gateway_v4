...
describe("Unit Tests - Admin Functions", () => {
  ...
  it("[ADMIN-001] should enable system when called by authorized authority", async () => {
    ...
    // Performance checks
    expect(cuUsedDisable).to.be.lessThan(CU_LIMITS.EXPECTED_ADMIN * 1.5, 
      `Disable used ${cuUsedDisable} CU, expected <${CU_LIMITS.EXPECTED_ADMIN * 1.5}`);
    expect(cuUsedEnable).to.be.lessThan(CU_LIMITS.EXPECTED_ADMIN * 1.5, 
      `Enable used ${cuUsedEnable} CU, expected <${CU_LIMITS.EXPECTED_ADMIN * 1.5}`);
    
    logSuccess("System successfully enabled by authority");
    console.log(`  System Status: ${gatewayAccount.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
  });
  ...
  it("[ADMIN-002] should disable system when called by authorized authority", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_ADMIN * 1.5, 
  ...
  it("[ADMIN-003] should reject unauthorized system enable/disable attempts", async () => {
    ...
    console.log(`  System Status Unchanged: ${gatewayAccount.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
  });
  ...
  it("[ADMIN-004] should handle multiple consecutive enable calls", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_ADMIN * 1.5, 
      `Multiple enables used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_ADMIN * 1.5}`);
    
    logSuccess("Multiple enable calls handled correctly");
    console.log(`  Final System Status: ${gatewayAccount.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
  });
  ...
  it("[ADMIN-005] should handle multiple consecutive disable calls", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_ADMIN * 1.5, 
      `Multiple disables used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_ADMIN * 1.5}`);
  ...
  it("[ADMIN-008] should allow operations to resume when system is re-enabled", async () => {
    ...
    const sendTx = await context.sendMessage(txId, recipient, destChainId, chainData, 1);
    const cuUsed = await logTransactionWithCU(
      sendTx, 
      context.connection, 
      context, 
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5, 
      `Resumed send used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5}`);
  ...
  it("[ADMIN-009] should work with different authority signers", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_ADMIN * 1.5, 
      `Authority signer used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_ADMIN * 1.5}`);
  ...
  it("[ADMIN-010] should maintain system state across multiple gateways", async () => {
    ...
    console.log(`  Gateway 1 (Chain ${context.chainId}): ${gateway1.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
    console.log(`  Gateway 2 (Chain ${context2.chainId}): ${gateway2.systemEnabled ? 'ENABLED' : 'DISABLED'}`);
  ...
  it("[ADMIN-012] should handle rapid enable/disable toggles", async () => {
    ...
    for (let i = 0; i < states.length; i++) {
      ...
      console.log(`  Toggle ${i + 1}: ${lastState ? 'ENABLED' : 'DISABLED'} -> ${targetState ? 'ENABLED' : 'DISABLED'}`);
