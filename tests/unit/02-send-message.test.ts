...
describe("Unit Tests - Message Sending", () => {
  ...
  it("[SEND-002] should handle maximum length recipient addresses", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5, 
      `Max recipient used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5}`);
  ...
  it("[SEND-003] should handle maximum length chain data", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5, 
      `Max chain data used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5}`);
  ...
  it("[SEND-004] should handle different destination chain IDs", async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      logSuccess(`Message sent to ${testCase.name} (Chain ID: ${testCase.chainId.toString()})`);
    }
  ...
  it("[SEND-005] should handle different confirmation requirements", async () => {
    ...
    for (let i = 0; i < confirmationTests.length; i++) {
      ...
      console.log(`  ✓ ${confirmations} confirmations: ${tx.substring(0, 8)}...`);
    }
  ...
  it("[SEND-007] should allow any valid sender to send messages", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5, 
      `Unauthorized send used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5}`);
    
    logSuccess("Any valid sender can send messages (no authorization restrictions)");
  });
  ...
  it("[SEND-008] should generate unique transaction IDs", async () => {
    ...
    for (let i = 0; i < numTxs; i++) {
      ...
      console.log(`  ✓ TX ${i + 1}: ID ${txId.toString()} -> ${tx.substring(0, 8)}...`);
    }
  ...
  it("[SEND-009] should reject empty chain data", async () => {
    ...
    await expectRevert(
      context.sendMessage(txId, recipient, destChainId, chainData, confirmations),
  ...
  it("[SEND-010] should validate minimum recipient address length", async () => {
    ...
    // Performance check
    expect(cuUsed).to.be.lessThan(CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5, 
      `Min recipient used ${cuUsed} CU, expected <${CU_LIMITS.EXPECTED_SEND_MESSAGE * 1.5}`);
